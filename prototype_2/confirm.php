...
<title>Booking Berhasil! — StayEase</title><?=$font?><?=$css?>
<style>
.confirm-wrap{padding:calc(var(--nav-h)+20px) 20px 100px;}
</style>
</head><body>
...
<div class="confirm-wrap">
  <div style="text-align:center;margin-bottom:24px;">
    ...
    <p class="confirm-sub">Konfirmasi dikirim ke <strong><?=htmlspecialchars($b['guest_email']??'kamu')?></strong></p>
  ...
  <div class="receipt">
    ...
    <div class="receipt-body">
      <div class="receipt-row"><span class="r-label">Nama Tamu</span><span class="r-val"><?=htmlspecialchars($b['guest_name'])?></span></div>
      ...
      <div class="receipt-row"><span class="r-label">Durasi</span><span class="r-val"><?=$b['nights']?> Malam</span></div>
    ...
    <div class="receipt-total"><span class="l">Total Dibayar</span><span class="r"><?=rp($b['total'])?></span></div>
  </div>
...
function shareNow(){if(navigator.share)navigator.share({title:'StayEase',text:'Booking <?=addslashes($b['hotel_name'])?> — ID: <?=$b['booking_id']?>'});else alert('Link disalin!');}
</script>
</body></html>
