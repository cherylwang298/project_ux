...
<header class="app-header">
  <div class="header-inner">
    ...
    <div style="flex:1;text-align:center;">
      <div style="font-size:14px;font-weight:800;letter-spacing:-0.3px;"><?=htmlspecialchars($location)?></div>
      <div style="font-size:10px;color:var(--c-text3);"><?=fds($checkin)?> – <?=fds($checkout)?> · <?=$nights?> malam</div>
    </div>
...
<div class="page-content">
  ...
  <div style="font-size:12px;color:var(--c-text2);font-weight:600;margin-bottom:14px;"><?=count($list)?> hotel ditemukan di <strong><?=htmlspecialchars($location)?></strong></div>
  ...
  <div class="hotel-list">
    ...
    <a href="hotel.php?id=<?=$h['id']?>&checkin=<?=$checkin?>&checkout=<?=$checkout?>" class="hotel-card">
      <div class="hotel-img-wrap">
        <div class="hotel-img-bg" style="background:<?=$h['grad']?>"><span style="font-size:66px;opacity:.28;"><?=$h['img_emoji']?></span></div>
      ...
      <div class="hotel-body">
        ...
        <div style="font-size:10px;color:var(--c-green);font-weight:700;margin-bottom:8px;">📋 <?=$h['cancel_policy']?></div>
    ...
    <div style="text-align:center;padding:48px 0;">
      ...
      <div style="font-size:17px;font-weight:800;">Tidak ada hasil</div>
      <p style="color:var(--c-text2);margin-top:8px;font-size:13px;">Coba ubah filter atau destinasi lain</p>
      <a href="index.php" class="btn-primary" style="margin-top:20px;display:inline-flex;width:auto;padding:12px 28px;">Kembali ke Beranda</a>
    </div>
    <?php endif; ?>
  </div>
</div>
  ...
  <div class="modal-sheet" onclick="event.stopPropagation()">
    ...
    <form method="GET">
      ...
      <div class="filter-group">
        ...
        <input type="range" name="budget" min="150000" max="1500000" step="50000" value="1500000"
          oninput="document.getElementById('bLabel').textContent='Rp '+Number(this.value).toLocaleString('id-ID')">
      </div>
      <div class="filter-group">
        <div class="filter-group-title">Rating Minimum</div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          <?php foreach([0,8.0,8.5,9.0] as $r): ?>
          <label style="display:flex;align-items:center;gap:6px;padding:8px 14px;border-radius:20px;border:1.5px solid rgba(0,0,0,0.10);background:rgba(255,255,255,0.65);cursor:pointer;font-size:12px;font-weight:700;">
            <input type="radio" name="min_rating" value="<?=$r?>" <?=$r==0?'checked':''?> style="accent-color:var(--a-red)">
            <?=$r==0?'Semua':'⭐ '.$r.'+'?>
          </label>
          <?php endforeach; ?>
        </div>
      ...
      <div class="filter-group">
        ...
        <?php foreach(['recommended'=>'✦ Rekomendasi','price_asc'=>'💰 Termurah','price_desc'=>'💸 Termahal','rating'=>'⭐ Rating Tertinggi'] as $v=>$l): ?>
