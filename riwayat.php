...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['cancel_ref'])) {
    ...
    session_write_close();
    header('Location: riwayat.php?updated=' . time(), true, 303); exit;
}
...
<main class="max-w-[1280px] mx-auto px-5 md:px-16 py-12">
  ...
  <div class="flex flex-wrap gap-2 mb-8 anim-fade-up">
    <?php foreach([
      'all'=>'All','accommodation'=>'Accommodations','flight'=>'Flights','active'=>'Active','completed'=>'Completed','cancelled'=>'Cancelled',
    ] as $val=>$lbl): ?>
    <a href="?filter=<?= $val ?>"
  ...
  <div class="space-y-4">
    ...
    <div class="bk-card rounded-2xl p-5 md:p-6 anim-fade-up delay-<?= min(400,$i*60) ?>">
      <div class="flex flex-col md:flex-row md:items-center gap-4">
        <!-- Type icon -->
        <div class="w-14 h-14 rounded-2xl bg-primary-fixed shrink-0 flex items-center justify-center">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-start justify-between gap-2 flex-wrap">
            <div>
              <p class="font-extrabold text-on-surface text-base"><?= h($iName) ?></p>
              <p class="text-xs text-outline font-mono mt-0.5"><?= h($b['booking_ref']) ?></p>
            </div>
            <span class="px-3 py-1.5 rounded-full border text-xs font-bold <?= $stxt.' '.$sbg.' '.$sborder ?> flex items-center gap-1 shrink-0">
              <span class="material-symbols-outlined text-[12px] icon-fill"><?= $b['status']==='confirmed'?'check_circle':($b['status']==='cancelled'?'cancel':'task_alt') ?></span>
              <?= ucfirst($b['status']) ?>
            </span>
          </div>
          <div class="flex flex-wrap gap-4 mt-3 text-sm">
            <?php if ($bt==='flight'): ?>
            <span class="flex items-center gap-1.5 text-on-surface-variant"><span class="material-symbols-outlined text-[15px]">calendar_month</span><?= $b['flight_date']?date('d M Y',strtotime($b['flight_date'])):'-' ?></span>
            <span class="flex items-center gap-1.5 text-on-surface-variant"><span class="material-symbols-outlined text-[15px]">airline_seat_recline_normal</span><?= ucfirst($b['seat_class']??'economy') ?></span>
            <span class="flex items-center gap-1.5 text-on-surface-variant"><span class="material-symbols-outlined text-[15px]">group</span><?= $b['guests'] ?> pax</span>
            <?php else: ?>
            <span class="flex items-center gap-1.5 text-on-surface-variant"><span class="material-symbols-outlined text-[15px]">login</span><?= $b['check_in']?date('d M Y',strtotime($b['check_in'])):'-' ?></span>
            <span class="flex items-center gap-1.5 text-on-surface-variant"><span class="material-symbols-outlined text-[15px]">logout</span><?= $b['check_out']?date('d M Y',strtotime($b['check_out'])):'-' ?></span>
            <span class="flex items-center gap-1.5 text-on-surface-variant"><span class="material-symbols-outlined text-[15px]">group</span><?= $b['guests'] ?> guests</span>
            <?php endif; ?>
            <span class="flex items-center gap-1.5 text-on-surface-variant"><span class="material-symbols-outlined text-[15px]">payments</span><?= h($payL[$b['payment_method']]??$b['payment_method']) ?></span>
          </div>
        </div>
        <!-- Amount + actions -->
        <div class="flex flex-col items-end gap-3 shrink-0">
          ...
          <div class="flex gap-2">
            <a href="<?= $resiLink ?>?ref=<?= urlencode($b['booking_ref']) ?>"
              class="px-4 py-2 bg-primary-fixed text-primary rounded-full text-xs font-bold hover:bg-primary hover:text-white transition-all flex items-center gap-1">
              <span class="material-symbols-outlined text-[14px]">receipt</span>E-Ticket
            </a>
            <?php if ($b['status']==='confirmed'): ?>
            <button onclick="showCancel('<?= h($b['booking_ref']) ?>')"
              class="px-4 py-2 bg-error-container text-error rounded-full text-xs font-bold hover:bg-error hover:text-white transition-all flex items-center gap-1">
...
<?= footer() ?>
<script>
function showCancel(ref){document.getElementById('cancelRefTxt').textContent=ref;document.getElementById('cancelRefIn').value=ref;document.getElementById('cancelModal').classList.remove('hidden');}
function closeCancel(){document.getElementById('cancelModal').classList.add('hidden');}
document.getElementById('cancelModal').addEventListener('click',e=>{if(e.target===e.currentTarget)closeCancel();});
</script>
</body></html>
